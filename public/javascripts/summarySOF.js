  ...
  var obv = smart.patient.api.fetchAll({
    ...
    query: {
      code: {
        ...
        'http://loinc.org|72166-2', // Tobacco smoker status
        'http://loinc.org|74013-4', // Alcohol intake
            ]
      }
    }
...
*/
  ...
  smart.patient.api.fetchAllWithReferences({type: 'AllergyIntolerance'}).then(function(results) {
    ...
    // console.log(results);
      if (allergy.category == "food") {
        $("#food-allergies-list").append("<p>" + allergy.substance.text + "</p>");
        $("#food-allergies-criticality").append("<p>" + allergy.criticality + "</p>");
        $("#food-allergies-verification").append("<p>" + allergy.status + "</p>");
        $("#food-allergies-onset").append("<p>" + allergy.onset + "</p>");
      } 
      else if (allergy.category == "medication") {
        $("#medical-allergies-list").append("<p>" + allergy.substance.text + "</p>");
        $("#medical-allergies-criticality").append("<p>" + allergy.criticality + "</p>");
        $("#medical-allergies-verification").append("<p>" + allergy.status + "</p>");
        $("#medical-allergies-onset").append("<p>" + allergy.onset + "</p>");
      }
    });
  });
  ...
  $.when(pt, obv, meds).done(
    function(patient, obv, prescriptions) {
      ...
      if (isDiabetic > 0) {
	       $("#has_diabetes_text").text("Yes");
      }
      else {
	       $("#has_diabetes_text").text("No");
      }
      if (hasHypertension) {
	       $("#has_hypertension_text").text("Yes");
      }
      else {
	       $("#has_hypertension_text").text("No");
      }
    ...
    smart.patient.api.fetchAllWithReferences({type: 'Observation'}).then(function(results) {
      results.forEach(function(observation){
        // console.log(results);    
        //NOTE: 
        if (observation.category.text == "Social History") {
          if (observation.code.text == "SHX Alcohol frequency"){          
            // console.log(observation.valueCodeableConcept.text);
            $("#alcohol-text").append("<p>" + observation.valueCodeableConcept.text + "</p>");
          }
          if (observation.code.text == "SHX Tobacco use"){
            // console.log(observation.valueCodeableConcept.text);
            $("#smoking-text").append("<p>" + observation.valueCodeableConcept.text + "</p>");
          }
        } 
      });
    });

    /* OLD CODE that uses appropriate codes - this does NOT WORK for 
      Cerner Data (use code above instead)
    */
    //       /* Get Smoking Status */
    //   var smoking = byCodes('72166-2');
    // smoking = sortObv(smoking);
    // var smokingFinal = smoking[0].valueCodeableConcept.coding.display;
    // if(smokingFinal == '-') {
    //   $("#smoking-text").text('N/A');
    // } else {
    //   $("#smoking-text").text(smokingFinal);
    // }

    //   /* Get Alcohol Status */
    //   var alcohol = byCodes('74013-4');
    // alcohol = sortObv(alcohol);
    // var alcoholFinal = alcohol[0].valueCodeableConcept.coding.display;
    // if(alcoholFinal == '-') {
    //   $("#alcohol-text").text('N/A');
    // } else {
    //   $("#alcohol-text").text(alcoholFinal);
    // } 
